...
const EditSalesOrder = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Changed to PUT method and included the ID in the URL for proper update
      const res = await axiosInstance.put(`/sales-order/update-sales-order/${id}`, formData, {
        headers: {
    ...
    try {
      ...
      if (response.data.statusCode === 200) {
        ...
        setFormData({
          ...
          salesChannel: salesOrderData.salesChannel || '',
          trackingNumber: salesOrderData.trackingNumber || '',
          shippingAddress: salesOrderData.shippingAddress || '',
          billingAddress: salesOrderData.billingAddress || '',
          customerPO: salesOrderData.customerPO || '',
          itemSku: salesOrderData.itemSku || '',
          packQuantity: salesOrderData.packQuantity || '',
          unitsQuantity: salesOrderData.unitsQuantity || 0,
          finalAmount: salesOrderData.finalAmount || 0,
          amount: salesOrderData.amount || 0
        });

        // Fetch pack types if itemSku exists
        if (salesOrderData.itemSku) {
          fetchProductsAvailablePackTypes(salesOrderData.itemSku);
        }
      }
  ...
  // Fetch pack types when itemSku changes
  useEffect(() => {
    if (formData.itemSku) {
      fetchProductsAvailablePackTypes(formData.itemSku);
    }
  }, [formData.itemSku]);
          ...
          <FormControl fullWidth>
            ...
            >
              <MenuItem value="" disabled>
                {packTypes.length === 0 ? 'Select product first...' : 'Select pack type'}
              </MenuItem>
