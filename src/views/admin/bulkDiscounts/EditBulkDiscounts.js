...
const EditBulkDiscount = () => { // FIXED: Changed component name
  ...
  const fetchBulkDiscountById = async () => {
    try {
      ...
      if (response.data.statusCode === 200) {
        ...
        const bulkDiscountData = response.data.data;
        setFormData({
          name: bulkDiscountData.name || '',
          product: bulkDiscountData.product?._id || bulkDiscountData.product || '',
          customers: bulkDiscountData.customers?.map(customer => 
            typeof customer === 'object' ? customer._id : customer
          ) || [],
          discount: bulkDiscountData.discount || 0
        });
      } else {
        setError(response.data.message);
        console.error('Error fetching bulk discount:', response.data.message);
      }
    } catch (error) {
      console.error('Error fetching bulk discount:', error);
      setError('Error fetching bulk discount: ' + error.message);
    }
  };

  useEffect(() => {
    if (id) {
      fetchBulkDiscountById();
    }
  }, [id]);
  ...
  const handleImportCsvFile = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post('/bulk-discounts/import-bulk-discounts', formDataForUpload, {
        headers: {
    ...
    <div>
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body1" color="textSecondary">
          Update the bulk discount details below
        </Typography>
      </Box>

      <Grid container spacing={2}>
        {/* Group Name */}
        <Grid item xs={12}> {/* FIXED: Changed size to item xs */}
        ...
        <Grid item xs={12} mt={3}>
          ...
          >
            {loading ? 'Updating...' : 'Update Bulk Discount'} {/* FIXED: Changed button text */}
          </Button>
          <Button
            variant="outlined"
            color="secondary"
            onClick={() => navigate('/dashboard/bulk-discounts/list')}
            disabled={loading}
            sx={{ mr: 2 }}
          >
            Cancel
...
export default EditBulkDiscount; // FIXED: Changed export name