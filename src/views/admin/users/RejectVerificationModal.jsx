...
import { getRejectionReasons } from '../../../services/verificationService';

const documentOptions = [
    { value: 'governMentId', label: 'Government ID' },
    { value: 'degreeCertificate', label: 'Degree Certificate' },
    { value: 'postGraduateDegreeCertificate', label: 'Post Graduate Certificate' },
    { value: 'licenseNumber', label: 'License Number' },
];

const RejectVerificationModal = ({ open, handleClose, handleReject, isSubmitting }) => {
    ...
    return (
        <Dialog open={open} onClose={!isSubmitting ? handleClose : undefined} fullWidth maxWidth="sm">
            <DialogTitle sx={{ pb: 1 }}>Reject Verification</DialogTitle>
            <DialogContent dividers>
                {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}

                <Typography variant="body2" color="textSecondary" sx={{ mb: 3 }}>
                    Please specify which document failed verification and provide a reason. This will be recorded and the user's verification will be marked as rejected.
                </Typography>
