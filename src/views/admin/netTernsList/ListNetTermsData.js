...
const ListNetTermsData = () => {
    ...
    const handleDeleteNetTerm = async () => {
        try {
            ...
            if (res.data.statusCode === 200) {
                ...
                // setError('Net term deleted successfully!');
                setTimeout(() => setError(''), 3000);
            } else {
                setError(res.data.message || 'Failed to delete net term');
            }
        } catch (error) {
            ...
            setError(error.response?.data?.message || error.message || 'Error deleting net term');
        } finally {
    ...
    // Edit net term
    const handleEditNetTerm = (id) => {
        navigate(`/dashboard/net-terms-list/edit/${id}`);
    };
                                    ...
                                    .map((row, index) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                    ...
                                                    <Box display="flex" gap={1}>
                                                        <Tooltip title="Edit">
                                                            <IconButton
                                                                size="small"
                                                                color="primary"
                                                                onClick={() => handleEditNetTerm(row._id)}
                                                            >
                                                        ...
                                                        <Tooltip title="Delete">
                                                            <IconButton
                                                                size="small"
                                                                color="error"
                                                                onClick={(e) => handleDeleteClick(e, row._id, row.netTermName)}
                                                            >
                                                ...
                                                {/* Net Term Name */}
                                                <TableCell sx={columnWidths.NetTermName}>
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={headCells.length + 2} align="center" sx={{ py: 3 }}>
                                            <Typography variant="h6" color="textSecondary">
                                                {search ? 'No net terms found matching your search' : 'No net terms found'}
                                            </Typography>
                                        </TableCell>
                                    </TableRow>
                                )}
...
export default ListNetTermsData;