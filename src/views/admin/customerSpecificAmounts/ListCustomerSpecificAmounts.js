  ...
  const handleExportCSV = async () => {
    try {
      const response = await axiosInstance.get(
        '/customer-secific-amounts/export-customer-specific-amounts',
        { responseType: 'blob' }
      );
      ...
      link.href = url;
      link.setAttribute("download", "customers-specific-amount-groups-export.csv");
...
const ListCustomerSpecificAmounts = () => {
  ...
  // Updated headCells based on requirements
  const headCells = [
    {
      id: 'name',
      label: 'Group Name',
  ...
  const fetchCustomersSpecificAmountGroups = async () => {
    try {
      ...
      console.error('Error fetching Customers Specific Amount Groups list:', error);
      setError(error.message);
    }
  };

  useEffect(() => {
    fetchCustomersSpecificAmountGroups();
  }, [id]);
  ...
  const handleSearch = (event) => {
    ...
    const filteredRows = tableData.filter((row) => {
      return (
        ...
        row?.basePrice?.toString().includes(searchValue) ||
        row?.actualPrice?.toString().includes(searchValue) ||
        row?.discountedPrice?.toString().includes(searchValue)
      );
    });
  ...
  const handleDeleteItemCustomerSpecificAmountGroups = async () => {
    try {
  ...
  const handleEditItemCustomerSpecificAmountGroups = (id) => {
    navigate(`/dashboard/customer-specific-amounts/edit/${id}`);
  };

  const handleGroupNameClick = (id) => {
    navigate(`/dashboard/customer-specific-amounts/customers-list/${id}`);
  };
    ...
    <Box>
      <Box>
        <EnhancedTableToolbar
          ...
          placeholder="Search Customer Specific Amounts"
        />
                      ...
                      >
                        <TableCell sx={stickyCellStyle}>
                          <Box display="flex" gap={1}>
                            <Tooltip title="Edit">
                              <IconButton size="small" color="primary" onClick={() => handleEditItemCustomerSpecificAmountGroups(row?._id)}>
                        ...
                        <TableCell align="center">
                          <Typography>
                            ${row?.actualPrice?.toFixed(2) || '0.00'}
                          </Typography>
                        </TableCell>

                        <TableCell align="center">
                          <Typography>
                            ${row?.discountedPrice?.toFixed(2) || '0.00'}
                          </Typography>
                        </TableCell>
      ...
      <DeleteConfirmationDialog
        ...
        onConfirm={handleDeleteItemCustomerSpecificAmountGroups}
        itemName={deleteDialog.itemName}
        isDeleting={deleteDialog.isDeleting}
        itemType={"Customer Specific Amount"}
      />
    </Box>
  );
};

export default ListCustomerSpecificAmounts;