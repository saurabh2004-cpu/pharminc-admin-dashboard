...
function descendingComparator(a, b, orderBy) {
  ...
  // Special handling for nested category object
  if (orderBy === 'category') {
...
const ListTable = ({
  ...
  const handleSearch = (event) => {
    ...
    if (isBrandsList) {
      const filteredRows = sourceData.filter((row) => {
        return (row.name.toLowerCase().includes(searchValue) || row.category?.name.toLowerCase().includes(searchValue));
      });
  ...
  const handleDeleteCategory = async (id) => {
    try {
      const res = await axiosInstance.delete(`/subcategory/delete-sub-category/${deleteDialog.itemId}`);
  ...
  return (
    <Box>
      ...
      <DeleteConfirmationDialog
        ...
        itemType={isBrandsList ? "Sub Category" : ""}
      />
